 ...
 <div class="col-lg-8 col-md-8 col-sm-12">
  <ol class="breadcrumb">
              <?php 
					include '../forms/connect.php';
								if(isset($_GET['cat'])){
										$get_cat = preg_replace('#[^0-9]#','',$_GET['cat']);
										$sql = "SELECT * FROM `categories` where category_id = '$get_cat' ";
										$sql_run = mysql_query($sql);
										while($rows1 = mysql_fetch_array($sql_run)){
											$cat_name = $rows1['category_name'];
										}
										print "
										<li><a href='#'>Home</a></li>
										<li class='active'><a href='#'>$cat_name</a></li>";
										
								}
		
			?>
  </ol>
  <div class="slick_slider2 slide_custom">
   <?php 
					include '../forms/connect.php';
								if(isset($_GET['cat'])){
										$get_cat = preg_replace('#[^0-9]#','',$_GET['cat']);
										$sql = "SELECT * FROM `reporter` where category_id = '$get_cat' LIMIT 0,3";
										$sql_run = mysql_query($sql);
										while($rows = mysql_fetch_array($sql_run)){
											...
											$title= $rows['title'];
											$story = $rows['story'];
											$story1 = substr($story,0,70);
											print "
										 <div class='single_featured_slide'> <a href='single.php?post=$post_id'><img src='../employees/new_images/$image'></a>
											  <h2><a href='single.php?post=$post_id'>$title</a></h2>
	...
	<h2> <span class="bold_line"><span></span></span> <span class="solid_line"></span> <span class="title_text">Latest Updates</span> </h2>
			  ...
			  <?php 
					include '../forms/connect.php';
								if(isset($_GET['cat'])){
										$get_cat = preg_replace('#[^0-9]#','',$_GET['cat']);
										$sql = "SELECT * FROM `reporter` where category_id = '$get_cat' LIMIT 0,1";
										$sql_run = mysql_query($sql);
										while($rows = mysql_fetch_array($sql_run)){
											$post_id = $rows['post_id'];
											$image= $rows['image'];
											...
											$story1 = substr($story,0,70);
											print "
										 <div class='archive_imgcontainer'><img src='../employees/new_images/$image'> </div>
											<div class='archive_caption'>
											  <h2><a href='single.php?post=$post_id'>$title</a></h2>
											  <p>$story1</p>
											</div>
											<a class='read_more' href='single.php?post=$post_id'><span>Read More</span></a>";
	...
	<ul class="small_catg popular_catg">
		<?php					include '../forms/connect.php';
										$get_cat = preg_replace('#[^0-9]#i','',$_GET['cat']);
										$sql = "SELECT * FROM `reporter` where category_id = $get_cat order by post_id desc LIMIT 1, 3 ";
										$sql_run = mysql_query($sql);
										while($rows = mysql_fetch_array($sql_run)){
											$post_id = $rows['post_id'];
											$title = $rows['title'];
											...
											$image = $rows['image'];
											print"<li>
												<div class='media fadeInDown'> <a class='media-left' href='single.php?post=$post_id'><img src='../employees/new_images/$image' ></a>
												  <div class='media-body'>
													<h4 class='media-heading'><a href='single.php?post=$post_id'>$title </a></h4>
													<div class='comments_box'> <span class='meta_date'>$post_time</span> <span class='meta_comment'><a href='single.php?post=$post_id'>No Comments</a></span> </div>
												  </div>
												</div>
											  </li>";
										}
